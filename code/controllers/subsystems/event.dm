...
/datum/controller/subsystem/event/proc/GetInteractWindow()
	...
	html += "<A align='right' href='byond://?src=\ref[src];pause_all=[!config.allow_random_events]'>Pause All - [config.allow_random_events ? "Pause" : "Resume"]</A>"
	...
	if(selected_event_container)
		...
		for(var/datum/event/E in active_events)
			...
			html += "<td><A align='right' href='byond://?src=\ref[src];view_variables=\ref[E]'>VV</A></td>"
			html += "</tr>"
		html += "</table>"
		html += "</div>"
...
/datum/controller/subsystem/event/Topic(href, href_list)
	...
	if(href_list["toggle_report"])
		...
		log_and_message_staff("has [report_at_round_end ? "enabled" : "disabled"] the round end event report.")
		...
		log_and_message_staff("decreased timer for [severity_to_string[EC.severity]] events by [decrease/600] minute(s).")
	else if(href_list["inc_timer"])
		var/datum/event_container/EC = locate(href_list["event"])
		var/increase = 60 * (10 ** text2num(href_list["inc_timer"]))
		EC.next_event_time += increase
		log_and_message_staff("increased timer for [severity_to_string[EC.severity]] events by [increase/600] minute(s).")
		...
		if(EM)
			log_and_message_staff("has queued the [severity_to_string[EC.severity]] event '[EM.name]'.")
	else if(href_list["pause"])
		var/datum/event_container/EC = locate(href_list["pause"])
		EC.delayed = !EC.delayed
		log_and_message_staff("has [EC.delayed ? "paused" : "resumed"] countdown for [severity_to_string[EC.severity]] events.")
	else if(href_list["pause_all"])
		config.allow_random_events = text2num(href_list["pause_all"])
		log_and_message_staff("has [config.allow_random_events ? "resumed" : "paused"] countdown for all events.")
		...
		if(delay && delay > 0)
			...
			log_and_message_staff("has set the interval modifier for [severity_to_string[EC.severity]] events to [EC.delay_modifier].")
		...
		var/datum/event_meta/EM = E.event_meta
		log_and_message_staff("has stopped the [severity_to_string[EM.severity]] event '[EM.name]'.")
		E.kill()
		...
		if(weight && weight > 0)
			...
			EM.weight = weight
			if(EM != new_event)
				log_and_message_staff("has changed the weight of the [severity_to_string[EM.severity]] event '[EM.name]' to [EM.weight].")
	else if(href_list["toggle_oneshot"])
		var/datum/event_meta/EM = locate(href_list["toggle_oneshot"])
		EM.one_shot = !EM.one_shot
		if(EM != new_event)
			log_and_message_staff("has [EM.one_shot ? "set" : "unset"] the oneshot flag for the [severity_to_string[EM.severity]] event '[EM.name]'.")
	else if(href_list["toggle_enabled"])
		var/datum/event_meta/EM = locate(href_list["toggle_enabled"])
		EM.enabled = !EM.enabled
		log_and_message_staff("has [EM.enabled ? "enabled" : "disabled"] the [severity_to_string[EM.severity]] event '[EM.name]'.")
		...
		EC.available_events -= EM
		log_and_message_staff("has removed the [severity_to_string[EM.severity]] event '[EM.name]'.")
		...
		log_and_message_staff("has added \a [severity_to_string[new_event.severity]] event '[new_event.name]' of type [new_event.event_type] with weight [new_event.weight].")
		new_event = new
	else if(href_list["clear"])
		var/datum/event_container/EC = locate(href_list["clear"])
		if(EC.next_event)
			log_and_message_staff("has dequeued the [severity_to_string[EC.severity]] event '[EC.next_event.name]'.")
