...
/obj/machinery/door/airlock/proc/isWireCut(wire)
...
/obj/machinery/door/airlock/requiresID()
	return !(src.isWireCut(WIRE_IDSCAN) || aiDisabledIdScanner)

/obj/machinery/door/airlock/check_access()
	if (isWireCut(WIRE_IDSCAN) || aiDisabledIdScanner)
...
/obj/machinery/door/airlock/proc/mainPowerCablesCut()
	return src.isWireCut(WIRE_MAIN_POWER1) || src.isWireCut(WIRE_MAIN_POWER2)

/obj/machinery/door/airlock/proc/backupPowerCablesCut()
	return src.isWireCut(WIRE_BACKUP_POWER1) || src.isWireCut(WIRE_BACKUP_POWER2)
...
/obj/machinery/door/airlock/proc/electrify(duration, feedback = 0)
	var/message = ""
	if(src.isWireCut(WIRE_ELECTRIFY) && arePowerSystemsOn())
	...
	switch (href_list["command"])
		...
		if("backup_power")
			if(!backup_power_lost_until)
				src.loseBackupPower()
		if("bolts")
			if(src.isWireCut(WIRE_DOOR_BOLTS))
		...
		if("lights")
			// Lights
			if(src.isWireCut(WIRE_BOLT_LIGHT))
...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user)
	...
	if(!repairing && isWelder(C) && !( operating > 0 ) && density)
		...
		return src.attack_hand(user)
	else if(istype(C, /obj/item/device/assembly/signaller))
		return src.attack_hand(user)
...
/obj/machinery/door/airlock/can_open(forced=0)
	...
	if(!forced)
		if(!arePowerSystemsOn() || isWireCut(WIRE_OPEN_DOOR))
			return 0
...
/obj/machinery/door/airlock/proc/unlock(forced=0)
	...
	if (!forced)
		if(operating || !src.arePowerSystemsOn() || isWireCut(WIRE_DOOR_BOLTS))
			return
