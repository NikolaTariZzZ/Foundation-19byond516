...
/area/proc/play_ambience(mob/living/L)
	...
	if(hum)
		if(!L.client.ambience_playing)
			...
			L.playsound_local(T,sound('sounds/ambience/shipambience.ogg', repeat = 1, wait = 0, volume = 30, channel = GLOB.ambience_sound_channel))
	else
		if(L.client.ambience_playing)
	...
	if(L.lastarea != src)
		if(LAZYLEN(forced_ambience))
			...
			L.playsound_local(T,sound(pick(forced_ambience), repeat = 1, wait = 0, volume = 60, channel = GLOB.lobby_sound_channel))
	/*	else	//stop any old area's forced ambience, and try to play our non-forced ones
			...
			forced_ambiance_list -= L */
	if(ambience.len && prob(40) && (world.time >= L.client.played + 2 MINUTES))
		L.playsound_local(T, sound(pick(ambience), repeat = 0, wait = 0, volume = 60, channel = GLOB.lobby_sound_channel))
