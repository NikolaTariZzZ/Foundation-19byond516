...
/obj/item/integrated_circuit/proc/set_pin_data(pin_type, pin_number, datum/new_data)
	if(islist(new_data))
		var/list/data_list = new_data
		for(var/i in 1 to length(data_list))
			var/datum/element = data_list[i]
			if (istype(element) && !isweakref(element))
				data_list[i] = weakref(element)
...
/datum/integrated_io/proc/get_data()
	if(islist(data))
		...
		for(var/i in 1 to length(data_list))
			var/datum/element = data_list[i]
			if(isweakref(element))
				var/weakref/dw = element
				element = dw.resolve()
	if(isweakref(data))
		return data.resolve()
	return data
