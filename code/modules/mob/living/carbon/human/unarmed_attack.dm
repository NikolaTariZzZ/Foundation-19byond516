...
/datum/unarmed_attack/proc/apply_effects(mob/living/carbon/human/user,mob/living/carbon/human/target,attack_damage,zone)
	...
	if(attack_damage >= 5 && armour < 1 && !(target == user) && stun_chance <= attack_damage * 5) // 25% standard chance
		switch(zone) // strong punches can have effects depending on where they hit
			...
			if(BP_GROIN)
				target.visible_message(SPAN_WARNING("[target] looks like \he is in pain!"), SPAN_WARNING("[(target.gender=="female") ? "Oh god that hurt!" : "Oh no, not your[pick("testicles", "crown jewels", "clockweights", "family jewels", "marbles", "bean bags", "teabags", "sweetmeats", "goolies")]!"]"))
				target.adjust_stutter((attack_damage/5) SECONDS)
				target.apply_effects(agony = attack_damage* 3, blocked = armour)
