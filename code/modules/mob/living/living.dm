...
/mob/living/proc/handle_pull_damage(mob/living/puller)
	...
	if(lying && prob(getBruteLoss() / 6))
		...
		if(prob(25))
			...
			visible_message(SPAN_DANGER("\The [src]'s [src.isSynthetic() ? "state worsens": "wounds open more"] from being dragged!"))
			. = TRUE
	if(src.pull_damage())
		if(prob(25))
			src.adjustBruteLoss(2)
			visible_message(SPAN_DANGER("\The [src]'s [src.isSynthetic() ? "state" : "wounds"] worsen terribly from being dragged!"))
			location.add_blood(src)
			. = TRUE
